...
class Nerual_Net:
    ...
    def nerual_net_building(self, X):
        n, m = len(X), len(X[0])
        self.weight_V = np.array(
            [[np.random.rand(1)[0] for _ in range(m)] for _ in range(self.num_node)])  # Vji shape:[num_node,m]
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(description='Nerual net')
    parser.add_argument('--lr', type=int, default=2, help='learning rate')
    parser.add_argument('--iteration', type=int, default=5, help='number of iteration')
    parser.add_argument('--num_node', type=int, default=3, help='number of hidden layer nodes')
    parser.add_argument('--DATA_PATH', type=str, default="e:/data set/a7a.test")
    args = parser.parse_args()

    test_ys, test_xs = parse_data(args.DATA_PATH)
    test_ys = [1 if x == 1 else 0 for x in test_ys]
    F = Nerual_Net(num_node=args.num_node, lr=args.lr, iteration=args.iteration)
